...
export class RegisterFormComponent implements OnInit {
  ...
  checkError(validator: string): boolean {
    ...
    if (!reqExp.test(controlValue) && validator !== 'minlength' && control.dirty) {
  ...
  ngOnInit(): void {
    this.registerForm = this.builder.group({
      ...
      password: this.builder.control('', [Validators.required, Validators.minLength(8), Validators.pattern(/[A-Z]/), Validators.pattern(/[0-9]/), Validators.pattern(/[a-z]/), Validators.pattern(/[!@#$%^&]/)])
    })
  }
